                    ...
                    <div class="table-responsive">
                      <table class="table table-striped" id="table-1">
                        ...
                        @foreach($orders as $order)
                          <tr>
                            <td><button data-toggle="modal" data-target="#vieworder-{{$order->id}}" class="btn btn-primary">Edit</button></td>
                                ...
                                <div class="modal-body">
                                  ...
                                  <p><b>Custom Design:</b> 
                                    @if(!empty($order->design))
                                        <a href="{{ asset('public/custom_designs/'.$order->design) }}">Click to View</a>
                                    @endif
                                </p>
                                  <p><b>Design for me?:</b> {{$order->help_me}}</p>

                                  <div class="text-center mt-3 mb-2">Update Status
                                  <form action="{{ route('update_order',$order) }}" method="post">@csrf 
                                    ...
                                    <button class="btn btn-outline-primary btn-sm mt-2">Go</button>
                                  </form>
                                  </div>
                                </div>
                              </div>
