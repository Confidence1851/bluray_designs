...
(function($) {
    ...
    function add_edit_product(url,mode){
        ...
        var PdesignPrice = $('#design_price').val();
        var Pcaption = $('#caption').val();

        if(Pname.length > 0 && Ptype !== null && PdesignPrice.length > 0 && Pcaption.length > 0){
        
            $.ajax({
                url: url,
                type: "POST",
                data: {name:Pname,type:Ptype,category:Pcat , design_price:PdesignPrice , caption:Pcaption},
                    success:function(data){
                        ...
                        if(mode === 'add'){
                            ...
                            $('#design_price').attr('disabled',true);
                    ...
                    error:function(){
                       alertNotify('An error occurred','red');
                    }
            });
