                                ...
                                <div class="bottom-article">
                                    <ul class="meta-post">
                                        @php($cmts = App\Comment::where('post_id', $post->id)->count())
                                            ...
                                            <li><i class="icon-comments"></i><a href="javascript:void(0)"><span
                                                        class="comment-count">{{ $cmts }}</span> Comments</a></li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        @else
                                            ...
                                            <div class="media">
                                                <a href="#" class="thumbnail pull-left"><img
                                                        src="{{ asset('public/web/img/avatar.png') }}" alt="" /></a>
                                                <div class="media-body">
                                                    <div class="media-content">
                                                        <h6><span>{{ date('D, M d Y', strtotime($comment->created_at)) }}</span>
                                                            {{ $name }}</h6>
                                                        <p>
                                                            {{ $comment->message }}
                                                        </p>
                                    ...
                                    <form id="commentform" action="{{ route('comment') }}" method="post">{{ csrf_field() }}
                                        <div class="row">
                                            ...
                                            <div class="span8 margintop10">
                                                <p>
                                                    <textarea rows="8" name="message" class="input-block-level" required
                                                        placeholder="*Your comment here"></textarea>
                                                </p>
                                                <p>
                                                    <button class="btn btn-theme margintop10" id="commentbtn" type="submit">Submit
                                                        comment</button>
                                                </p>
                                            </div>
                                        </div>
                                    ...
                                    <div class="widget">
                                        ...
                                        <ul class="recent">
                                            @foreach ($relates as $relate)
                                                <li>
                                                    <img src="{{ asset('public/post_images/' . $relate->image) }}" class="pull-left"
                                                        alt="{{ $relate->title }}" />

                                                    <h6><a
                                                            href="{{ route('blogpost', ['id' => $relate->id, 'slug' => $relate->slug]) }}">{{ $relate->title }}</a>
                                                    </h6>
